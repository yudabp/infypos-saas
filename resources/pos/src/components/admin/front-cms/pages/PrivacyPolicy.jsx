...
const PrivacyPolicy = () => {
    ...
    useEffect(() => {
        if (pagesData?.privacy_policy) {
            setData(pagesData?.privacy_policy);
        }
    }, [pagesData?.privacy_policy]);

    const onSubmit = () => {
        dispatch(updatePages({ privacy_policy: data }));
    };
    ...
    const disabled = pagesData?.privacy_policy === data

    return (
        <MasterLayout>
            <TabTitle title={placeholderText("front-cms.privacy-policy.title")} />
            <div className='card'>
                        ...
                        <div className="w-100 d-flex">
                            ...
                            <div className="w-100 pages-quill">
                                <ReactQuill
                                    ...
                                    value={data || ""}
                                    onChange={(content) =>
                                        onChangeInput({ target: { name: "privacy_policy", value: content } })
                                    }
                                />
